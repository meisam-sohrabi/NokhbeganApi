...
namespace NokhbeganApi.Controllers
{
    public class VerifyPaymentController : Controller
    {
        ...
        public VerifyPaymentController(IPay pay, IHttpClientFactory httpClientFactory, UserManager<T_CustomUser> userManager)
        {
            _pay = pay;
        ...
        public async Task<IActionResult> Verify()
        {
            ...
            if (string.IsNullOrWhiteSpace(id))
                return BadRequest("اطلاعات جلسه کاربری یافت نشد.");

            var user = await _userManager.FindByIdAsync(id);

            if (user == null)
                return Unauthorized("کاربر یافت نشد.");
            var payment = await _pay.GetPayment(user.Id);
            ...
            if (!string.IsNullOrWhiteSpace(status) && (!string.IsNullOrWhiteSpace(trackId) && (!string.IsNullOrWhiteSpace(orderId))))
            {
                ...
                try
                {
                    ...
                    if (resultResponse == "100")
                    {
                        ...
                        var isSuccess = true;
                        await _pay.UpdatePayment(user.Id, isSuccess, message, statusMessage);
